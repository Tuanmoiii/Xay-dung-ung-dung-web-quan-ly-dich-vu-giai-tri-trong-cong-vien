	...
	<div class="mx-auto max-w-7xl">
		...
		<div class="mt-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
			<div class="flex items-center gap-2">
				<form method="get" class="relative w-full max-w-xs flex">
					...
					<input name="q" value="<?= htmlspecialchars($q ?? '') ?>" class="w-full rounded-l-lg border-slate-300 bg-white py-2 pl-10 pr-2 text-sm text-slate-800 placeholder-slate-400 focus:border-primary focus:ring-primary dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:placeholder-slate-500" placeholder="Tìm kiếm dịch vụ..." type="text"/>
					<button type="submit" class="h-9 rounded-r-lg bg-primary px-3 text-white">Tìm</button>
				</form>
			</div>
				...
				<div class="inline-block min-w-full align-middle">
					...
					<table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
						...
						<tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-background-dark">
							...
							<?php else: if (!empty($schedules)): foreach ($schedules as $i => $sch): ?>
								<tr>
									...
									<td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
										<a class="text-primary hover:text-primary/80 mr-3" href="edit.php?id=<?= $sch['schedule_id'] ?>&service_id=<?= $sch['service_id'] ?? $service_id ?>">Sửa</a>
							...
							<?php endforeach; else: ?>
								<?php if ($q !== ''): ?>
									<tr><td colspan="5" class="text-center py-6">Không tìm thấy lịch cho "<?= htmlspecialchars($q) ?>".</td></tr>
								<?php else: ?>
