...
/**
 * Tìm kiếm đặt vé theo tên khách hàng hoặc mã đặt vé
 */
function searchBookings($keyword) {
    $conn = getDbConnection();
    $like = "%$keyword%";
    $sql = "SELECT b.*, c.full_name, c.email, c.phone, s.date, s.start_time, s.end_time, sv.service_name
            FROM bookings b
            JOIN customers c ON b.customer_id = c.customer_id
            JOIN schedules s ON b.schedule_id = s.schedule_id
            JOIN services sv ON s.service_id = sv.service_id
            WHERE b.booking_ref LIKE ? OR c.full_name LIKE ?
            ORDER BY b.created_at DESC";
    $stmt = mysqli_prepare($conn, $sql);
